...
object Interpreter {
  ...
  def eval(expr: Expr, declarations: List[FDeclaration]): M[Either[Boolean, Integer]] =
    expr match {
      ...
      case App(name, arg) =>
        ...
        bind(eval(arg, declarations)) { value =>
          bind(get()) { s1 =>
            bind(put(declareVar(fdecl.arg, value.fold(_ => throw new IllegalArgumentException("Function argument must be integer"), identity), s1))) { s2 =>
